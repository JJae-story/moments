...
public class PostService {
  ...
  // 게시글 등록
  @Transactional
  public PostResponseDto createPost(Long userId, Long groupId, Long albumId,
      PostCreateRequestDto postCreateRequestDto) {

    Album album = albumRepository.findById(albumId)
        .orElseThrow(() -> new CustomException(ALBUM_NOT_FOUND));

    boolean hasFiles = validateFiles(postCreateRequestDto.getFiles());
  ...
  // 게시글 수정
  @Transactional
  public PostResponseDto updatePost(Long userId, Long groupId, Long postId, Long albumId,
    ...
    // 삭제할 이미지 처리
    if (postUpdateRequestDto.getDeleteImageIds() != null
        && !postUpdateRequestDto.getDeleteImageIds().isEmpty()) {
      List<PostImage> postImages = post.getPostImages();

      for (int i = postImages.size() - 1; i >= 0; i--) {
        PostImage postImage = postImages.get(i);
        Long albumImageId = postImage.getAlbumImage().getId();

        if (postUpdateRequestDto.getDeleteImageIds().contains(albumImageId)) {
          awsS3Service.deleteFile(albumImageId);
          postImages.remove(i);
        }
      }
    }
    ...
    int newImageFileCount = (postUpdateRequestDto.getFiles() == null) ? 0 : postUpdateRequestDto.getFiles().size();
    validateFileCount(remainingImageFileCount, newImageFileCount);

    // 새로운 이미지 추가
    if (validateFiles(postUpdateRequestDto.getFiles())) {
  ...
  public List<PostResponseDto> getPostsByDateRange(Long userId, Long groupId,
    ...
    if (weeks != 1 && weeks != 2) {
      throw new CustomException(INVALID_WEEK_SELECTION);
    }
